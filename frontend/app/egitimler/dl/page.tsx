...
export default function DeepLearningHistory() {
    ...
    const fetchTrainings = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/api/deep-learning/training?username=${username}`
        );
  ...
  const handleDelete = async (id: number) => {
    try {
      const username = localStorage.getItem("username");
      const response = await fetch(
        `http://localhost:8000/api/deep-learning/training/${id}?username=${username}`,
        {
          method: "DELETE",
        }
      );
      ...
      <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
        <div className="flex flex-col space-y-1.5 p-6">
          <h3 className="text-2xl font-semibold leading-none tracking-tight">
            Derin Öğrenme Eğitimlerim
          </h3>
          <p className="text-sm text-muted-foreground">
            Tüm derin öğrenme model eğitimlerinizin geçmişi
          </p>
        </div>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            router.push(`/egitimler/dl/${training.id}`)
                          }
                        >
